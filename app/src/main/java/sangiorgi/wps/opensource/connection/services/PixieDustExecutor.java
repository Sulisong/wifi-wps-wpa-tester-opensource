...
import sangiorgi.wps.opensource.connection.commands.*;
import sangiorgi.wps.opensource.connection.models.PixieDustParameters;
import sangiorgi.wps.opensource.connection.utils.WpsPixieExtractor;
...
public class PixieDustExecutor {
  ...
  public PixieDustExecutor(Context context) {
    this.context = context;
    this.config = new CommandConfig(context, false, 30);
  }
  ...
  private WpsResult attemptWpsConnection(String bssid, String pin) {
    try {
      ...
      // Create a temporary executor for WPS connection
      WpsExecutor wpsExecutor = new WpsExecutor(context);

      // Execute WPS connection with discovered PIN
      WpsResult result = wpsExecutor.executeWpsConnection(bssid, pin).get(30, TimeUnit.SECONDS);
