...
import sangiorgi.wps.opensource.connection.handlers.ConnectionHandler;
 ...
 *
 * <p>Uses @AssistedInject for proper dependency injection while supporting runtime parameters.
 */
public class ConnectionService {
  ...
  /**
   * Factory interface for creating ConnectionService instances. Hilt generates the implementation
   * automatically.
   */
  @AssistedFactory
  public interface Factory {
    ConnectionService create(
        @Assisted("networkToTest") NetworkToTest networkToTest,
        @Assisted("callback") ConnectionUpdateCallback callback);
  }

  /**
   * Creates a ConnectionService with injected dependencies.
   *
   * @param context Application context (injected)
   * @param executor WpsExecutor instance (injected via Hilt)
   * @param connectionHandlerFactory Factory for creating ConnectionHandler (injected via Hilt)
   * @param networkToTest Network to test (runtime parameter)
   * @param callback Callback for updates (runtime parameter)
   */
  @AssistedInject
  ...
  public void cleanup() {
    stopActiveHandler();
    stateManager.cancel();
    // Cancel running operations but don't cleanup the executor
    // WpsExecutor is a singleton managed by Hilt and shared across connections
    executor.cancel();
  }
